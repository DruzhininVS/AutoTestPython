import yaml
from sshcheckers import ssh_checkout, upload_files, ssh_getout, getout, ssh_checkout_negative
...
class TestPositive:
   ...
   def test_step7(self, clear_folders, make_files, make_subfolder, start_time):
       res = []
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z a "
           "{}/arx -t{}".format(data["folder_in"], data["folder_out"], data["type"]), "Everything is Ok"))
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {};"
           " 7z x arx.{} -o{} -y".format(data["folder_out"], data["type"], data["folder_ext2"]), "Everything is Ok"))
       for item in make_files:
...
class TestNegative:
   ...
   def test_ng_step1(self, make_folders, make_bad_arx, start_time):
       self.save_log(start_time, "log10.txt")
       assert ssh_checkout_negative(data["ip"], data["user"], data["passwd"], "cd {}; 7z e arxbad.{} -o{} -y".format(data["folder_out"], data["type"],
                                                                       data["folder_ext"]), "ERROR:"), "test_ng_1 FAIL"

   def test_ng_step2(self, make_bad_arx, start_time):
       self.save_log(start_time, "log11.txt")
       assert ssh_checkout_negative(data["ip"], data["user"], data["passwd"], "cd {}; 7z t arxbad.{}".format(data["folder_out"],
                                                               data["type"]), "ERROR:"), "test_ng_2 FAIL"

   def test_uninstall(self, start_time):
       self.save_log(start_time, "log12.txt")
       res = []
       ...
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "echo '{}' | "
                                                                         "sudo -S dpkg -s {}".format(data["passwd"],
                                                                                                     data["pkgname"]),
                               "Status: deinstall ok"))
       assert all(res), "test_uninstall FAIL"